...
//! compiler-checked symbol, `image(res::images::app_logo)`, instead of a bare string: a typo is
//! a build error, the resource is guaranteed bundled, and the available names autocomplete.
//! Adding or removing a file under `resource/` regenerates on the next build.
fn main() {
